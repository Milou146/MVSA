    ...
    if ply.GasMaskEquiped then
        if not ply.GASMASK_SpamDelay or ply.GASMASK_SpamDelay < CurTime() then
            ...
            if ply:GetNWBool("GasMaskSet") then
                ...
                net.WriteBool(false)
                net.Send(ply)
                sql.Query("UPDATE mvsa_player_character SET GasMaskSet = 0 WHERE SteamID64 = " .. tostring(ply:SteamID64()) .. " AND RPName = " .. "'" .. ply.RPName .. "'")
                ...
                net.WriteBool(true)
                net.Send(ply)
                sql.Query("UPDATE mvsa_player_character SET GasMaskSet = 1 WHERE SteamID64 = " .. tostring(ply:SteamID64()) .. " AND RPName = " .. "'" .. ply.RPName .. "'")
